...
export class Edit_userComponent implements OnInit {
  ...
  exist = 0;
            ...
            if(this.id)
            {
                this.service.getUserDetails(this.id).subscribe(response =>{
                    if(response['status']=='success')
                    {
                        ...
                        this.registration_date = user[0].registration_date
    ...
    onUpdate() {
        this.movieService
          .edit_movie(this.username, this.firstname, this.lastname, this.registration_date, this.phoneno, this.email, this.password, this.id,this.exist)
          .subscribe(response => {
            if (response['status'] == 'success') {
                alert('user-updated')
